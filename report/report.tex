...
\medskip\noindent
Second, the diagnosis of \emph{that} failure was also wrong. It was attributed
to a code-generation capability ceiling at 85M, with the conclusion that teacher
distillation could not help. The real cause was a defaulted argument in this
project's own SFT script: every repository demonstration had been built on the
single-token-mutation tier, so the policy had been taught to copy a file back
with one token changed --- which is precisely what it did. Rebuilding the
demonstrations on the \code{stub} tier and re-running SFT for $11$ minutes took
$\mathrm{pass@}1$ from $\mathbf{0\%}$ to $\mathbf{9.6\% \pm 1.9}$ and
$\mathrm{pass@}12$ from $\mathbf{0\%}$ to $\mathbf{40\%}$, with the model
writing correct function bodies from bare stubs.

\medskip\noindent
The conclusion is therefore about method rather than capacity. A benchmark's
difficulty and a training set's difficulty are the same hidden parameter: the
flattering benchmark made the model look better than it was, and the flattering
training set made it look worse. Both were defaults chosen for convenience, and
both silently became definitions.
\end{abstract}
...
7. SFT on \code{stub} demos & $\mathrm{pass@}1$ $0\% \rightarrow 9.6\%$; $\mathrm{pass@}12$ $0\% \rightarrow 40\%$ & \YES \\
\bottomrule
\end{tabular}
...
\section{The capability ceiling that was not one}
\label{sec:notaceiling}

Section~\ref{sec:hardness} concluded that the $0/120$ reflected a
code-generation \emph{capability} limit at 85M, and that teacher distillation
therefore could not help. That conclusion was wrong, and the cause was a
defaulted argument in this repository's own SFT script:

\begin{center}
\code{scns = build\_scenarios(base\_tasks, seed=1)}\quad\emph{--- difficulty
defaults to \code{"mutate"}}
\end{center}

Every repository demonstration the policy had ever seen was a single-token
mutation, where the correct fix is \emph{write the file back with one token
changed}. So that is what it learned, and the probe caught it precisely: in
$236$ of $240$ episodes it wrote the \emph{unchanged stub} back to disk. It had
mastered copying, not coding. The $93$--$98\%$ ``wrote a file'' rate was
measuring a copy.

Adding \code{--tool-difficulty} and rebuilding the demonstrations on the
\code{stub} tier --- where the demonstration writes the \textbf{whole reference
body} --- then re-running SFT for $1{,}500$ steps ($11$ minutes, one GPU):

\begin{table}[h]
\centering\small
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{\code{stub} tier, 240 episodes} & \textbf{mutate-trained demos} & \textbf{stub-trained demos} \\
\midrule
$\mathrm{pass@}1$          & $0.0\%$ \;(0/240)       & $\mathbf{9.6\% \pm 1.9}$ \;(23/240) \\
$\mathrm{pass@}12$         & $0.0\%$ \;(0/20 tasks)  & $\mathbf{40.0\%}$ \;(8/20 tasks) \\
any partial credit         & $0.0\%$                 & $\mathbf{11.2\%}$ \\
left the stub unchanged    & $\mathbf{236/240}$      & $99/240$ \\
wrote valid Python         & $1/240$                 & $\mathbf{92/240}$ \\
\bottomrule
\end{tabular}
\caption{The same checkpoint lineage, the same tasks, the same probe. The only
change is which tier the \emph{demonstrations} were built on.}
\end{table}

\noindent Correct code, written from a bare stub, reward $1.00$:
...
\begin{retraction}{The method lesson, learned twice in one day, in opposite directions}
A benchmark's difficulty and a training set's difficulty are the same hidden
parameter, and an unexamined parameter is an assumption either way. The
flattering \emph{benchmark} made the model look better than it was
($70\% \rightarrow 0\%$). The flattering \emph{training set} made it look worse
than it was ($0\% \rightarrow 9.6\%$). Both were defaults chosen because they
were the easiest thing to implement, and both silently became definitions ---
of the task, and then of the model's ceiling.

The operational rule: \textbf{before declaring a capability limit, print one
demonstration and read it.} Had that been done once, the second error would not
have happened -- and it was made while writing up the first.
\end{retraction}

\noindent Two revisions follow for the earlier claims.

\medskip\noindent\textbf{``RL cannot bootstrap a convention the policy never
emits'' survives, with a sharper statement.} It is not enough to add
demonstrations; they must demonstrate \emph{the target behaviour}. Demos of an
adjacent, easier task actively train the wrong skill, which is what happened
here. And $\mathrm{pass@}12 = 40\%$ supplies exactly the precondition RL was
missing: groups now carry reward spread instead of being dropped as
zero-variance.

\medskip\noindent\textbf{Teacher distillation is no longer ruled out on
capability grounds} --- better demonstrations demonstrably move this model.
It remains not built here for a separate and unchanged reason: using a Claude
subscription to generate training data for a different model is a restricted use
under Anthropic's terms. That was flagged when the idea was first raised.
...
\begin{itemize}
\item \textbf{Terminating a GPU instance does not shut it down if an Auto
Scaling Group owns it.} On teardown the instance was terminated, every
checkpoint was verified present in S3, and billing was reported stopped --- and
three minutes later a \emph{new} \code{g7.2xlarge} was running, because the
self-healing spot ASG built for unattended runs had replaced it, doing exactly
its job. Scale every ASG to $0/0/0$ \emph{first}, then terminate, then cancel
the spot request, then verify all three are empty. A second ASG was worse: it
sat at $\text{min}=\text{desired}=1$ with \emph{zero} instances because PRO
6000 capacity was unavailable, and would have launched a GPU whenever capacity
freed, potentially weeks later. \textbf{An ASG with desired ${>}0$ and no
instances is not idle; it is waiting.} Before terminating a resurrected box,
check whether it wrote to S3 --- the launch template auto-resumes, so a
replacement can overwrite good checkpoints (here it wrote nothing).
...
\medskip\noindent\textbf{The ceiling I reported was my own training data.}
An 85M model at $240$ tokens/param learns the agentic \emph{tool workflow}
convincingly --- $93$--$98\%$ write rate, $86\%$ self-verification, $0.07\%$
malformed calls. I then reported that it \emph{cannot} learn to write code
($0/240$ from a stub) and called that a capacity limit. It was not: the
demonstrations had taught copying. With demonstrations of the actual target it
writes correct bodies from bare stubs at $9.6\%$ $\mathrm{pass@}1$ and $40\%$
$\mathrm{pass@}12$. The honest remaining limit is that $9.6\%$ is low, not that
it is zero --- and the difference between those two claims is the whole
difference between ``this scale cannot do it'' and ``this run had not taught
it''.
